...
def apply_crf(unary, img):
    ...
    d.addPairwiseBilateral(sxy=20, srgb=15, rgbim=img.astype(np.uint8), compat=compat[1],
...
def cluster_features(features, saliency_maps, images, masks_fg=None, crf=True):
    ...
    for k in features:
        ...
        fg_features.append(k[dist < 0.75])
        bg_features.append(k[dist >= 0.75])
...
def estimate_foreground_mask(features, saliency_maps, images, thres=0.1):
    # collect foreground and background features
    fg_features = []
    bg_features = []
    for s, k in zip(saliency_maps, features):
        fg_features.append(k[s.flatten() >= thres])
        bg_features.append(k[s.flatten() < thres])
    fg_features = torch.cat(fg_features, axis=0).cpu().numpy()
    bg_features = torch.cat(bg_features, axis=0).cpu().numpy()
    ...
    masks_fg = []
    for i, (k, img) in enumerate(zip(features, images)):
        h, w = img.shape[2]//cfg.ps, img.shape[3]//cfg.ps
        all_features = np.ascontiguousarray(k.cpu().numpy()) # (h*w) x d
        dist, labels = kmeans.index.search(all_features, 1)
        dist_bg, _ = kmeans_bg.index.search(all_features, 1)
        is_fg = dist < dist_bg
