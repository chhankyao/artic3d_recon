...
class Model(nn.Module):
    def __init__(self, device, num_imgs, skeleton_path):
        ...
        self.soft_renderer = Renderer(device, shader='soft_phong', light='ambient')
        ...
        self.text_img = nn.Parameter(torch.zeros(cfg.nb, 3, 128, 128).float().to(self.device))
        ...
        self.diffusion = StableDiffusion(self.device)
        for p in self.diffusion.parameters():
            p.requires_grad = False
        self.text_prompt = 'A photo of %s' % cfg.animal_class.split('-')[0]
        self.text_z = self.diffusion.get_text_embeds([self.text_prompt]*2)
    ...
    def center_verts(self, verts):
        bs = verts.shape[0]
        center = verts.mean(2).mean(1)
        verts -= center[:,None,None,:]
        return verts
    ...
    def get_texture(self, inputs, outputs, mode=None):
        ...
        if mode == 'sample':
            ...
            u = torch.arctan2(inputs['uvs'][:,0], inputs['uvs'][:,2])/np.pi
            v = torch.arctan2(inputs['uvs'][:,1], (inputs['uvs'][:,0]**2+inputs['uvs'][:,2]**2).sqrt())*2/np.pi
            uv = torch.stack([u,v], 1)[None,None] # 1 x 1 x V x 2
            verts_color = torch.cat([F.grid_sample(self.text_img[i:i+1], uv, align_corners=True) for i in range(cfg.nb)], 2)
            verts_color = torch.sigmoid(verts_color) # B x 3 x P x V
            verts_color = verts_color.reshape(-1,3,cfg.nb*nv).permute(0,2,1) # B x P*V x 3
            verts_color = ((verts_color + verts_color[:,verts_sym,:])/2).view(-1,cfg.nb,nv,3)
        return verts_color # B x P x V x 3

    def calculate_losses(self, inputs, outputs, losses, weights, params):  
        uvs, faces = self.meshes[5].get_uvs_and_faces(gitter=False)
        verts5 = uvs[None].repeat(cfg.nb,1,1)
        verts5 += torch.cat([f(uvs[None], params['stop_at']) for f in self.f_parts], 0)
        ...
        for k in weights:
            ...
            if k == 'feat' and weights[k] > 0:
                ...
                loss = normal_loss(verts5[None], faces)
            elif k == 'lap' and weights[k] > 0:
                loss = laplacian_loss(verts5[None], faces)
            elif k == 'inflat' and weights[k] > 0:
                loss = inflation_loss(outputs['verts'][:1,:1], inputs['faces'])
                
            # texture reconstruction losses
            elif k == 'text' and weights[k] > 0:
                img = self.hard_renderer.render(outputs['verts'].detach(), inputs['faces'], outputs['verts_color'])[...,:3]
                loss = mse_loss(img.permute(0,3,1,2), inputs['images'], (inputs['masks']>0).float())

            elif k == 'text_init' and weights[k] > 0:
                with torch.no_grad():
                    text_sampled = self.get_texture(inputs, outputs, mode='sample')
                loss = mse_loss(outputs['verts_color'], text_sampled)
            
            # DASS loss
            elif k == 'dass':
                bone_rot = np.random.rand() * self.bone_rot[cfg.instance_idx]
                rot = torch.cat([self.rot_id[None,None], bone_rot[None] + self.bone_rot_rest[None]], 1)
                joints_can, joints_rot = self.skeleton.transform_joints(rot, scale=self.bone_scale)
                verts = self.transform_verts(inputs['uvs'], joints_can, joints_rot, True, params['stop_at'])
                verts = self.center_verts(verts)
                img = self.soft_renderer.render(verts, inputs['faces'], outputs['verts_color'], mv=True, rand=True)[...,:3]
                loss = self.diffusion.get_img_grad(self.text_z, img.permute(0,3,1,2), weight=5e-5, g=12, t=250, n=3)
    ...
    def optimize(self, variables, inputs, params, weights, output_prefix=None):
        ...
        self.soft_renderer.set_sigma_gamma(sigma, gamma)
    ...
    def train_all(self, inputs):
        ...
        weights = {'feat':1.0, 'pose_prior':0.01, 'sil':0.1, 'shape_prior':0.2, 'lap':0.02, 'norm':0.02, 'inflat':1e-4}
        ...
        weights = {'feat':1.0, 'global_prior':0.01, 'can_prior':0.1, 'pose_prior':0.01,
                   'sil':0.1, 'shape_prior':0.2, 'lap':0.02, 'norm':0.02, 'inflat':1e-4}
    ...
    def optimize_instance(self, inputs):        
        print("========== Initializing %s instance %d shape... ========== " % (cfg.animal_class, cfg.instance_idx))
        var = []
        for i, f in enumerate(self.f_parts):
            var += f.parameters()
        ...
        print("========== Initializing %s instance %d texture... ========== " % (cfg.animal_class, cfg.instance_idx))
        var = [self.text_img]
        params = {'n_iters':200, 'lr':0.1, 'lev':3, 'mesh_lev':5, 'deform':True, 'sigma':1e-5, 'gamma':1e-5, 'text':'uv'}
        weights = {'text_init':1.0}
        self.optimize(var, inputs, params, weights, 'opt_inst_')
        
        print("========== Optimizing %s instance %d shape and texture... ========== " % (cfg.animal_class, cfg.instance_idx))
        var = [self.text_img]
        for i, f in enumerate(self.f_parts):
            var += f.parameters()
        params = {'n_iters':100, 'lr':1e-2, 'lev':3, 'mesh_lev':4, 'deform':True, 'sigma':1e-5, 'gamma':1e-5, 'text':'uv'}
        weights = {'dass':1.0, 'text':0.1, 'sil':0.1, 'instance':1.0, 'lap':10.0, 'norm':10.0}
        self.optimize(var, inputs, params, weights, 'opt_diff_')
    ...
    def save_results(self, inputs, params, losses=None, prefix=''):
        ...
        if prefix == '' or prefix == 'opt_diff_':
            print('Rendering instances...')
    ...
    def save_model(self, model_path):
        ...
        state_dict_filtered = {}
        for k in state_dict:
            if 'diffusion' not in k and 'text_z' not in k:
                state_dict_filtered[k] = state_dict[k]
        torch.save(state_dict_filtered, model_path)
    ...
    def save_texture(self, model_path):
        torch.save(self.text_img, model_path)
        
    def load_texture(self, model_path):
        self.text_img = torch.load(model_path, map_location=self.device)
