...
def preprocess_data():        
    ...
    with open(osp.join(cfg.lassie_ann_dir, '%s.csv'%cfg.animal_class), 'r') as f:
        ...
        for i, row in enumerate(reader):
            ...
            bb = process_bbox(left, top, width, height, scale_width=2.0, scale_height=2.0)
            bbox_gt.append(bb)
    ...
    # occluded images
    if cfg.e_lassie:
        with open(osp.join(cfg.lassie_ann_dir, '%s_occ.csv'%cfg.animal_class), 'r') as f:
            reader = csv.DictReader(f)
            for i, row in enumerate(reader):
                img_id = int(row['id'])            
                img_file = osp.join(cfg.lassie_img_dir, '%s/input_%d.png'%(cfg.animal_class, img_id))                    
                ...
                keypoints = np.zeros((14,3))
                if 'kps' in row:
                    kps = json.loads(row['kps'].replace("'", '"'))
                    for kp in kps:
                        kp = kp['value']
                        kp_name = kp['keypointlabels'][0]
                        kp_id = kp_indices[kp_mapping[kp_name]]
    ...
    if cfg.diff_input:
        ...
        print("Estimating foreground masks...")
        masks_fg = estimate_foreground_mask(features, saliency, images, thres=0.03)
        masks_fg = [F.interpolate(m, cfg.input_size, mode='nearest') for m in masks_fg]
        
        print("Enhancing images...")
        diffusion = StableDiffusion(cfg.device)
        text_z = diffusion.get_text_embeds(['A photo of %s' % cfg.animal_class])
        diffused = []
        for i in tqdm(range(len(images))):
            img = images[i] # * masks_fg[i] + (1-masks_fg[i].repeat(1,3,1,1)) * 0.5
            img += torch.randn_like(img) * (1-masks_fg[i].repeat(1,3,1,1)) * 0.05
            diff = diffusion.enhance_img(text_z, img.clamp(0,1))
            diffused.append(diff)
        diffused = [F.interpolate(img, cfg.crop_size, mode='bilinear', align_corners=False) for img in diffused]
        images = diffused
    ...
    _, _, V = torch.pca_lowrank(feat_sal, q=cfg.d_feat, center=True, niter=3)
